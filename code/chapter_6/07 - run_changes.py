"""Complexity Economics and Sustainable Development
   Chapter 6 source code
    ...
    Description: performs simulations with alternative amounts of government
        expenditure
...
for country in countries:
    ...
    for change in changes:
        ...
        Bs = ppi.get_dirsbursement_schedule(Bs, B_dict, years_forward*sub_periods) * (1+change/100)
        ...
        sols = np.array(Parallel(n_jobs=parallel_processes, verbose=0)(delayed(ppi.run_ppi_parallel)\
                (I0, alphas, alphas_prime, betas, A=A, R=R, qm=qm, rl=rl, Imax=Imax, Imin=Imin, Bs=Bs, B_dict=B_dict) for itera in range(sample_size)))
        ...
        dff.to_csv(home+"/data/chapter_6/simulation/changes/"+country+'_'+str(change)+".csv", index=False)
