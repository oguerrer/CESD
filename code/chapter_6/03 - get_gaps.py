...
for group in df.group.unique():
    for indi in df.seriesCode.unique():
        new_row = [group, int(indi.split('_')[0][3::]), indi, np.mean(data[group][indi])]
        new_rows.append(new_row)


dfn = pd.DataFrame(new_rows, columns=['group', 'sdg', 'seriesCode', 'meanGap',])
dfn.sort_values(by=['group', 'sdg', 'seriesCode'], inplace=True)
dfn.to_csv(home+"/data/chapter_6/results/gaps.csv", index=False)
