"""Complexity Economics and Sustainable Development
   ...
   Description: runs the simulations on the substitutability of remittances per right
...
for right in rights:
    ...
    index = np.where((df.right==right))[0]
    ...
    while impact > .1 or impact < 0:
        ...
        if impact > .1 or impact < 0:
            ...
            Bsc[rights_progs[right]] *= 1+5*impact/100
        
    bud_subs.append( (right, Bsc[rights_progs[right]].sum()) )



dff = pd.DataFrame(bud_subs, columns=['right', 'budget'])
dff.to_csv(home+'/data/chapter_12/simulation/substitution/remittances/right_level.csv', index=False)
