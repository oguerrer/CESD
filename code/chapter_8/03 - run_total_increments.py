"""Complexity Economics and Sustainable Development
   ...
   Description: simulates total expenditure increments directed to 
       the rule of law
...
for country in countries:
    ...
    deltaB_range = np.linspace(0, 1, 100)
    for deltaB in deltaB_range:
        
        print(country, deltaB)
        ...
        (I0=I0, alphas=alphas, alphas_prime=alphas_prime, betas=betas, A=A, R=R, bs=bs,
         qm=qm, rl=rl, Imax=Imax, Imin=Imin, Bs=B_n, B_dict=B_dict) for itera in range(sample_size))
        ...
        all_deltaBs += (np.ones(values.shape[0])*deltaB).tolist()
    
    arr1 = np.append(np.array([all_DB, all_rl, all_qm, all_deltaBs]).T, np.array(all_P), axis=1)
    arr2 = np.append(arr1, all_DP, axis=1)
    df_data = pd.DataFrame( arr2, 
                           columns=['DB', 'rl', 'qm', 'deltaB']+
    ...
    df_data.to_csv(home+'/data/chapter_8/simulation/total_changes/'+country+'.zip', index=False, compression='zip')
