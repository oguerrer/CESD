...
for country in countries:
    ...
    # Simulation with frontier 1
    frontier = np.zeros(N)
    frontier[R==1] = 1
    sols = np.array(Parallel(n_jobs=parallel_processes, verbose=0)(delayed(ppi.run_ppi_parallel)\
            (I0, alphas, alphas_prime, betas, A=A, R=R, qm=qm, rl=rl, Imax=Imax, Imin=Imin, 
              Bs=Bs, B_dict=B_dict, frontier=frontier) for itera in range(sample_size)))
    ...
    dff.to_csv(home+"/data/chapter_7/simulation/frontier/"+country+".csv", index=False)
    
    # Simulation with frontier 0.9
    frontier = np.zeros(N)
    frontier[R==1] = .9
    sols = np.array(Parallel(n_jobs=parallel_processes, verbose=0)(delayed(ppi.run_ppi_parallel)\
            (I0, alphas, alphas_prime, betas, A=A, R=R, qm=qm, rl=rl, Imax=Imax, Imin=Imin, 
    ...
    dff = pd.DataFrame(np.hstack([[[c] for c in dft.seriesCode], aver_Is]), columns=['seriesCode']+list(range(aver_Is.shape[1])))
    dff.to_csv(home+"data/chapter_7/simulation/frontier90/"+country+".csv", index=False)
